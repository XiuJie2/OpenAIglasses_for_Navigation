...
# ========= 影像源配置 =========
VIDEO_SOURCE = "webcam"  # 可选："webcam" 或 "esp32"
WEBCAM_INDEX = 0         # 笔记本摄像头索引（通常为0）
INPUT_W, INPUT_H = 600, 480  # 输入分辨率
...
# ======== 影像源类 =========
class VideoSource:
    def __init__(self, source_type="webcam", index=0, width=640, height=480):
    ...
    def initialize(self):
        ...
        if self.source_type == "webcam":
            print(f"[VIDEO] 正在打开笔记本摄像头 (索引: {self.index})...")
            self.cap = cv2.VideoCapture(self.index)
            if not self.cap.isOpened():
                print(f"[ERROR] 无法打开摄像头索引 {self.index}")
                return False
            
            # 设置分辨率
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, self.width)
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, self.height)
            print(f"[VIDEO] 笔记本摄像头已打开，分辨率: {self.width}x{self.height}")
            
        elif self.source_type == "esp32":
            print("[VIDEO] 使用ESP32影像源")
            try:
                import bridge_io
                # ESP32通过bridge_io获取帧
                self.bridge_io = bridge_io
                # 测试获取一帧
                test_frame = bridge_io.wait_raw_bgr(timeout_sec=2)
                if test_frame is None:
                    print("[ERROR] 无法从ESP32获取帧，请检查连接")
                    return False
                print("[VIDEO] ESP32连接成功")
            except ImportError:
                print("[ERROR] 未找到bridge_io模块，请检查安装")
                return False
            except Exception as e:
                print(f"[ERROR] ESP32初始化失败: {e}")
                return False
        
        self.initialized = True
        return True
    
    def get_frame(self, timeout_sec=0.5):
        """获取一帧图像"""
        ...
        if self.source_type == "webcam":
            ...
            return self.bridge_io.wait_raw_bgr(timeout_sec=timeout_sec)
    
    def send_visualization(self, vis_frame):
        """发送可视化帧（仅ESP32需要）"""
        if self.source_type == "esp32" and self.initialized:
            try:
                self.bridge_io.send_vis_bgr(vis_frame)
            except Exception as e:
                print(f"[WARNING] 发送可视化帧失败: {e}")
    
    def send_ui_message(self, message):
        """发送UI消息（仅ESP32需要）"""
        if self.source_type == "esp32" and self.initialized:
            try:
                self.bridge_io.send_ui_final(message)
            except Exception as e:
                print(f"[WARNING] 发送UI消息失败: {e}")
    
    def release(self):
        """释放资源"""
        if self.source_type == "webcam" and self.cap is not None:
            self.cap.release()
            print("[VIDEO] 摄像头已释放")
        self.initialized = False
...
def draw_text_cn(img_bgr, text, xy, font_size=20, color=(255,255,255), stroke=None, ui_hint=True):
    ...
    cv2.putText(img_bgr, text, (x, y + font_size), cv2.FONT_HERSHEY_SIMPLEX, 
                font_size/24.0, color, 2, cv2.LINE_AA)
...
def main(headless: bool = False, prompt_name: str = None, stop_event=None, 
         video_source="webcam", webcam_index=0, width=600, height=480):
    ...
    # 初始化影像源
    print(f"[INIT] 初始化影像源: {video_source}")
    video = VideoSource(source_type=video_source, index=webcam_index, 
                       width=width, height=height)
    if not video.initialize():
        print("[ERROR] 影像源初始化失败")
        return
    ...
    try:
        while True:
            ...
            # 从影像源获取帧
            frame = video.get_frame(timeout_sec=0.5)
            if frame is None:
                # 没取到帧就继续等
                if headless:
                    cv2.waitKey(1)
            ...
            if MODE == "SEGMENT":
                ...
                if candidate_masks:
                    ...
                    if contours:
                        ...
                        if M["m00"] != 0:
                            ...
                            # 目标标签：保持就地标注
                            draw_text_cn(vis, "目标", (cx + 15, cy - 5), font_size=16, color=(0, 255, 0), ui_hint=False)
                ...
                if detected_object and last_detected_mask is not None:
                    ...
                    if remaining > 0:
                        # 显示倒计时（移动到左下角，前端风格）
                        draw_text_cn(vis, f"检测到物体，{remaining:.1f}秒后自动锁定", (10, 100), font_size=16, color=(255, 255, 255))
                    ...
                    draw_text_cn(vis, "分割中... 等待检测到物体", (10, 100), font_size=16, color=(200, 200, 200))
                ...
                if flash_start_time is not None and flash_mask is not None:
                    ...
                    if elapsed < flash_duration:
                        ...
                        # 显示提示文字（左下角）
                        draw_text_cn(vis, "正在锁定目标...", (10, 100), font_size=18, color=(255, 255, 0)) 
                    else:
                                ...
                                if poly_center:
                                    ...
                                    if not center_reached:
                                        if is_centered:
                                            ...
                                            video.send_ui_message("✓ 物品已居中！")
                                            draw_text_cn(vis, "✓ 物品已居中！", (10, 60), font_size=18, color=(0, 255, 0)) 
                                        else:
                                            # 显示引导文字
                                            msg = f"请将物品移到画面中心: {direction}"
                                            video.send_ui_message(msg)
                                            draw_text_cn(vis, msg, 
                                                       (10, 40), font_size=18, color=(255, 255, 255))
                                            ...
                                            draw_text_cn(vis, f"距离: {distance}px", 
                                                       (10, 60), font_size=16, color=(200, 200, 200))
                                        ...
                                        # 已经居中，显示成功信息
                                        video.send_ui_message("✓ 物品已成功移到中心！")
                                        draw_text_cn(vis, "✓ 物品已成功移到中心！", 
                                                   (10, 60), font_size=18, color=(0, 255, 0))
                                        
                                        # 等待1秒后进入手部追踪模式
                                        if t_now - last_center_guide_time > 1.0:
                                            print("[CENTER] 进入手部追踪模式")
                                            video.send_ui_message("进入手部追踪模式")
                                            MODE = "TRACK"
                                            # 保持当前的光流追踪状态
                                else:
                                    # 多边形中心计算失败，显示警告
            ...
            draw_text_cn(vis, f"FPS: {fps:.1f}", (10, 40), font_size=16, color=(0, 255, 0)) 
            ...
            # 显示影像源信息
            source_text = f"影像源: {video_source}"
            if video_source == "webcam":
                source_text += f" (索引: {webcam_index})"
            draw_text_cn(vis, source_text, (W - 200, 40), font_size=16, color=(200, 200, 200), ui_hint=False)

            # 发送可视化帧到影像源（如果是ESP32）
            video.send_visualization(vis)
                ...
                if key in (27, ord('q')):
                    ...
                    if video_source == "webcam":
                        ...
                        video = VideoSource(source_type=video_source, index=webcam_index, 
                                           width=width, height=height)
                        if not video.initialize():
                            print("[ERROR] 无法切换到ESP32模式，保持当前模式")
                            video_source = "webcam"
                            video = VideoSource(source_type=video_source, index=webcam_index, 
                                              width=width, height=height)
                            video.initialize()
                        ...
                        video = VideoSource(source_type=video_source, index=webcam_index, 
                                           width=width, height=height)
                        video.initialize()
                    print(f"[VIDEO] 已切换到 {video_source} 模式")
...
if __name__ == "__main__":
    # 在文件开头设置 VIDEO_SOURCE 变量来选择影像源
    # VIDEO_SOURCE = "webcam"  # 使用笔记本摄像头
    # VIDEO_SOURCE = "esp32"   # 使用ESP32摄像头
    
    main(video_source=VIDEO_SOURCE, webcam_index=WEBCAM_INDEX, 
